...
import { Bell, ClipboardList, LayoutDashboard, Trophy, type LucideIcon } from "lucide-react";
import { useEffect, useState, type ReactNode } from "react";
import { clearAuthToken, decodeAuthRole, getAuthToken } from "@/lib/auth";
import { ApiError } from "@/lib/api";
import { getTeamManagerDashboard } from "@/lib/team-manager-api";

const NAV_ITEMS = [
  { href: "/team-manager", label: "Overview", icon: LayoutDashboard },
  { href: "/team-manager/demo-queue", label: "Demo queue", icon: ClipboardList },
  { href: "/team-manager/notifications", label: "Notifications", icon: Bell },
] satisfies Array<{ href: string; label: string; icon: LucideIcon }>;

const POLL_MS = 20_000;

export function TeamManagerShell({
  title,
  children,
  subtitle,
}: {
  ...
  useEffect(() => {
    ...
    async function refreshUnread() {
      try {
        const data = await getTeamManagerDashboard();
        if (!cancelled) setUnreadNotifications(data.summary.unreadNotifications);
      } catch (err) {
    ...
    <div className="min-h-svh bg-slate-100">
      <header className="sticky top-0 z-30 border-b border-white/10 bg-brand-950/90 backdrop-blur-md">
        <div className="mx-auto flex h-16 max-w-7xl items-center justify-between px-4 sm:px-6">
          <Link href="/team-manager" className="flex items-center gap-2">
          ...
          <div className="flex items-center gap-2">
            <Link
              ...
              className="relative rounded-lg border border-white/20 p-2 text-white hover:bg-white/10"
            >
              <Bell className="h-4 w-4" />
              {unreadNotifications > 0 ? (
                <span className="absolute -right-1 -top-1 flex h-4 min-w-4 items-center justify-center rounded-full bg-brand-amber-500 px-1 text-[10px] font-bold text-brand-950">
